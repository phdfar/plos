...
def get_model(name: str):
    if 'dino' in name:
        ...
        #model = torch.load('Tupdated_checkpoint.pth')
        
        new_weight = torch.load('checkpoint.pth')

        old_weight = model.state_dict()

        for key in new_weight['teacher'].keys():
            gkey = key.replace('backbone.','') 
            if  gkey in old_weight.keys():
                #print('gkey',gkey)
                old_weight[gkey] = new_weight['teacher'][key]

        model.load_state_dict(old_weight)

        model.fc = torch.nn.Identity()
        val_transform = get_transform(name)
        patch_size = model.patch_embed.patch_size
        num_heads = model.blocks[0].attn.num_heads
        #print('>>>>>>>>>>>>>>>>>>>>>> ',patch_size,num_heads)
    else:
