#%%writefile /kaggle/working/MaskFreeVIS/detectron2/detectron2/engine/train_loop.py
# %load /kaggle/working/MaskFreeVIS/detectron2/detectron2/engine/train_loop.py
...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self):
        ...
        if self.zero_grad_before_forward:
            self.optimizer.zero_grad()
        with autocast(enabled=False):#(dtype=self.precision):
            loss_dict = self.model(data)
            if isinstance(loss_dict, torch.Tensor):
                losses = loss_dict
                loss_dict = {"total_loss": loss_dict}
