...
def extract_instance_eigs(feats_root,masks_root,export_root,std_threshold=60,aother='0'):
    ...
    for path in tqdm(feats_path):
        ...
        if os.path.exists(out)==False:
            ...
            os.system('rm tmpfeat.obj ')
            with open('tmpfeat.obj', 'wb') as fp:
               pickle.dump([feats_sep,out,aother], fp)
            
            if aother=='0':
                #output  = our_affinty(feats_sep)
                os.system('timeout 60s python affinty.py')
            else:
